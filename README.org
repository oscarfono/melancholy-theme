...
#+ATTR_ORG: :width 800
[[./screens/melancholy-org-agenda-preview.png]]
...
looks best when the preferred fonts are installed.

Run =M-x melancholy-check-fonts= after loading the theme to see what is
installed and where to get anything that is missing.
...
| VCS           | magit, diff-hl                                            |
| LSP           | lsp-mode, lsp-ui, eglot                                   |
| Search        | consult, deadgrep, rg.el, wgrep                           |
| Syntax check  | flycheck, flymake                                         |
| Org           | org-mode, org-agenda                                      |
| Email/RSS     | notmuch, gnus, elfeed, erc                                |
| UI            | which-key, nerd-icons, all-the-icons, indent-guide        |
| Themes/frames | tab-bar, tab-line, perspective                            |
| Terminal      | term (full 16-colour ANSI set), vterm                     |
| Languages     | JS/TS, Python, Go, Rust, Zig, C/C++, Bash, Terraform      |
| Tree-sitter   | Generic treesit faces + language-specific ts-mode faces   |
...
If you are adding faces for a new package, please follow the existing colour
logic:
